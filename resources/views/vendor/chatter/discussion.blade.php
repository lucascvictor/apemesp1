...
<div id="chatter" class="discussion">
	...
	<div id="chatter_header" style="background-color:{{ $discussion->color }}">
		<div class="container">
			...
			<h1>{{ $discussion->title }}</h1><span class="chatter_head_details">Postado em {{ Config::get('chatter.titles.category') }}<a class="chatter_cat" href="/{{ Config::get('chatter.routes.home') }}/{{ Config::get('chatter.routes.category') }}/{{ $discussion->category->slug }}" style="background-color:{{ $discussion->category->color }}">{{ $discussion->category->name }}</a></span>
		</div>
	</div>
	...
	@if (count($errors) > 0)
	    <div class="chatter-alert alert alert-danger">
	    	<div class="container">
	    		<p><strong><i class="chatter-alert-danger"></i> {{ Config::get('chatter.alert_messages.danger') }}</strong> Por favor corrija os erros abaixo:</p>
		        <ul>
	        ...
	        <div class="col-md-12">
		                		...
		                		<span class="chatter_posts">
		                			@if(!Auth::guest() && (Auth::user()->id == $post->user->id))
		                				<div id="delete_warning_{{ $post->id }}" class="chatter_warning_delete">
		                					<i class="chatter-warning"></i>Você tem certeza que deseja deletar esta resposta?
		                					<button class="btn btn-sm btn-danger pull-right delete_response">Sim apague isso</button>
		                					<button class="btn btn-sm btn-default pull-right">Não, obrigado</button>
		                				</div>
			                			<div class="chatter_post_actions">
			                				<p class="chatter_delete_btn">
			                					<i class="chatter-delete"></i> Deletar
			                				</p>
	            ...
	            @if(!Auth::guest())
	            	...
	            	<div id="new_response">
				            ...
				            <form id="chatter_form_editor" action="/{{ Config::get('chatter.routes.home') }}/posts" method="POST">
						    	...
						    	<div id="editor">
									@if( $chatter_editor == 'tinymce' || empty($chatter_editor) )
										<label id="tinymce_placeholder">Adicione o conteudo para a discussão aqui.</label>
						...
						<div id="discussion_response_email">
							<button id="submit_response" class="btn btn-success pull-right"><i class="chatter-new"></i> Enviar resposta</button>
							@if(Config::get('chatter.email.enabled'))
								<div id="notify_email">
									<img src="/vendor/devdojo/chatter/assets/images/email.gif" class="chatter_email_loader">
									<!-- Rounded toggle switch -->
									<span>Notificar-me quando alguém responder</span>
									<label class="switch">
				...
				@else
					...
					<div id="login_or_register">
						<p>Por favor <a href="/{{ Config::get('chatter.routes.home') }}/login">login</a> or <a href="/{{ Config::get('chatter.routes.home') }}/register">registre-se</a> para deixar uma resposta.</p>
					</div>
