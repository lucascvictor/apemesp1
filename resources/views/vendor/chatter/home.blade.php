	    ...
	    <div class="row">
	    	...
	    	<div class="col-md-3 left-column">
	    		...
	    		<div class="chatter_sidebar">
					...
					<a href="/{{ Config::get('chatter.routes.home') }}"><i class="chatter-bubble"></i> Todas as categorias {{ Config::get('chatter.titles.discussions') }}</a>
				        	...
				        	<li>
				        		<a class="discussion_list" href="/{{ Config::get('chatter.routes.home') }}/{{ Config::get('chatter.routes.discussion') }}/{{ $discussion->category->slug }}/{{ $discussion->slug }}">
					        		...
					        		<div class="chatter_middle">
					        			...
					        			<span class="chatter_middle_details">Postado Por: <span data-href="/user">{{ ucfirst($discussion->user->{Config::get('chatter.user.database_field_with_user_name')}) }}</span> {{ \Carbon\Carbon::createFromTimeStamp(strtotime($discussion->created_at))->diffForHumans() }}</span>
	...
	<div id="new_discussion">
    	...
    	<form id="chatter_form_editor" action="/{{ Config::get('chatter.routes.home') }}/{{ Config::get('chatter.routes.discussion') }}" method="POST">
        	<div class="row">
	        	<div class="col-md-7">
		        	<!-- TITLE -->
	                <input type="text" class="form-control" id="title" name="title" placeholder="Titulo da {{ Config::get('chatter.titles.discussion') }}" v-model="title" value="{{ old('title') }}" >
	            </div>
        	...
        	<div id="editor">
        		@if( $chatter_editor == 'tinymce' || empty($chatter_editor) )
					<label id="tinymce_placeholder">Adicione o conteúdo para sua discussão aqui</label>
            ...
            <div id="new_discussion_footer">
            	<input type='text' id="color" name="color" /><span class="select_color_text">Selecione uma cor para essa discussão (opcional)</span>
            	<button id="submit_discussion" class="btn btn-success pull-right"><i class="chatter-new"></i> Criar {{ Config::get('chatter.titles.discussion') }}</button>
